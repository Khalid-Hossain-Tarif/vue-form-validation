    ...
    <div class="bg-slate-100 rounded-xl p-10">
        <form action="">
            ...
            <div class="form-group">
                ...
                <div class="error-message" v-show="number !='' && !isValidNumber">Input a 11 digits number. Ex: 017******** / +880**********</div>
            </div>

            <div class="form-group">
                <label class="primary-form-label" for="email">Email</label>
                <input class="primary-input-field" type="email" name="" id="email" placeholder="Enter your email" v-model="email" required>
                <div class="error-message" v-show="email !='' && !isValidEmail">Email didn't matched!</div>
            </div>

            <div class="form-group">
                <label class="primary-form-label" for="password">Password</label>
                <input class="primary-input-field" type="password" name="" id="password" placeholder="Enter your password" v-model="password" required>
                <div class="error-message" v-show="password !='' && !isStrongPassword">Weak Password</div>
            </div>

            <div class="form-group">
                <label class="primary-form-label" for="confirm-password">Confirm Password</label>
                <input class="primary-input-field" type="password" name="" id="confirm-password" placeholder="Confirm your password" v-model="confirmPassword" required>
                <div class="error-message" v-show="confirmPassword !='' && !isConfirmPassword">Password didn't match!</div>
            </div>
